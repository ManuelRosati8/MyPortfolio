  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        const glowAlpha = Math.max(0, 1 - distToMouse / MOUSE_GLOW) * (dark ? 0.5 : 0.35);

        ctx.beginPath();
        ctx.arc(n.x, n.y, 2 + glowAlpha * 4, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(${dotColor},${0.12 + glowAlpha})`;
        ctx.fill();
      }
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < CONNECTION_DIST) {
            const alpha = (1 - dist / CONNECTION_DIST) * (dark ? 0.08 : 0.05);
            const da = Math.sqrt((mx - a.x) ** 2 + (my - a.y) ** 2);
            const db = Math.sqrt((mx - b.x) ** 2 + (my - b.y) ** 2);
            const mouseBoost = Math.max(0, 1 - Math.min(da, db) / MOUSE_GLOW) * 0.15;
            ctx.beginPath();
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
